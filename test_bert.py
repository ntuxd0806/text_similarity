...
# 資料數 (Shape of the data)
print(f"Total train samples : {train_df.shape[0]}")
print(f"Total validation samples: {valid_df.shape[0]}")
print(f"Total test samples: {valid_df.shape[0]}")
print()

#  查看第一列資料 (Print first row in our train data)
print(f"Sentence1: {train_df.loc[1, 'sentence1']}")
print(f"Sentence2: {train_df.loc[1, 'sentence2']}")
print(f"Similarity: {train_df.loc[1, 'similarity']}")
print()

# 資料集中有些許空值，先跳過 (We have some NaN entries in our train data, we will simply drop them.)
...
class BertSemanticDataGenerator(tf.keras.utils.Sequence):
    ...
    def __getitem__(self, idx):
        ...
        # With BERT tokenizer's batch_encode_plus batch of both the sentences are encoded together and separated
        # by [SEP] token.
...
# 這個策略適用於一個機器上有多個不同的 device (多個 CPU 和 GPU)
...
# Freeze the BERT model to reuse the pre-trained features without modifying them.
...
This is an optional last step where `bert_model` is unfreezed and retrained with a very low learning rate.
This can deliver meaningful improvement by incrementally adapting the pretrained features to the new data.
"""
